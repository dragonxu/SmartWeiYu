...
public class ArcProgress extends ProgressBar {
    ...
    /** 设置参数
     * @param context
     * @param mBoardWidth 进度条的宽
     * @param radius 半径
     */
    public void setBoardAndRadius(Context context,int mBoardWidth,int radius) {
        this.mBoardWidth = mBoardWidth;
        this.mRadius = radius;

        mUnmProgressColor = DEFAULT_mUnmProgressColor;
        mProgressColor = context.getResources().getColor(R.color.colorProgress);
        mTickWidth = DEFAULT_mTickWidth;
        mTickDensity = 3;
        mArcbgColor = DEFAULT_mUnmProgressColor;
        mTickDensity = Math.max(Math.min(mTickDensity,MAX_DENSITY),MIN_DENSITY);
        mBgShow = false;
        mDegree = 180;
        mStylePogress = STYLE_ARC;
        boolean capRount = true;
        mArcPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mArcPaint.setColor(mArcbgColor);
        if(capRount)
            mArcPaint.setStrokeCap(Paint.Cap.ROUND);
...
//            int heightSize = (int) (mRadius*2+mBoardWidth*2);
            int heightSize = (int) (mRadius +mBoardWidth);
...
//        float y = mArcRectf.right / 2 + mBoardWidth / 2;
        float x = mArcRectf.right / 2 + mBoardWidth;
        float y = mArcRectf.right / 2 + mBoardWidth;
        if (mOnCenter != null) {
...
//        mArcRectf = new RectF(mBoardWidth,mBoardWidth,mRadius*2 - mBoardWidth,mRadius*2 - mBoardWidth);
        mArcRectf = new RectF(mBoardWidth,mBoardWidth,mRadius*2 - mBoardWidth,mRadius*2 - mBoardWidth);
//        mArcRectf = new RectF(mBoardWidth,mBoardWidth,mRadius*2-(mBoardWidth*2),mRadius*2-mBoardWidth);
