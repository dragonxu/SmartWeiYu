...
import com.example.smartouwei.view.ViewfinderView;
...
public class CaptureActivity extends Activity implements Callback {
	...
	private void initView() {
		...
		findViewById(R.id.mo_scanner_light).setOnClickListener(
				new OnClickListener() {
					public void onClick(View v) {
						Log.e("CapureActivity","isGetCamera:~~~~~~~~~"+ isGetCamera);
						if (isGetCamera) {
							light();
						}
					}
	...
	protected void light() {
		...
		if (flag == true) {
			flag = false;
			Log.e("CapureActivity", "??闪光??");
			// ??闪光??
			CameraManager.get().openLight();

		} else {
			flag = true;
			Log.e("CapureActivity", "关闪光灯");;
			// 关闪光灯
	...
	private void photo() {
		...
		Intent wrapperIntent = Intent.createChooser(innerIntent,"Choose the qr code image");
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							if (result == null) {
								Log.i("123", "   -----------");
								Looper.prepare();
								Toast.makeText(getApplicationContext(),CaptureActivity.this.getString(R.string.erweima_youwu), 0)
										.show();
								finish();
								Looper.loop();
							} else {
								...
								Intent data = new Intent(BaseVolume.QrCode_NewDEVICE);
...
//							setResult(300, data);
	...
	protected Result scanningImage(String path) {
		...
		scanBitmap = BitmapFactory.decodeFile(path, options);
		// 将bitmap当成图片保存到手机根目录
		saveMyBitmap("bitmap",scanBitmap);
...
//		setResult(300, data);
	...
	/**
	 * //TODO: TAOTAO 将bitmap由RGB转换为YUV //TOOD: 研究??
	 *
	 * @param bitmap
	 *            转换的图??
	 * @return YUV数据
	...
	private boolean isGetCamera = true;
